...
func (h SeekerAfterCompileHook) AfterCompile(compiler *libbuildpack.Stager) error {
	...
	err, useSensorDownload := h.shouldDownloadSensor()
...
func (h SeekerAfterCompileHook) shouldDownloadSensor() (error, bool) {
	...
	seekerVersionOnlySupportSensorDownload := true
	var err error
	...
	if err != nil {
		...
		seekerVersionOnlySupportSensorDownload = h.isSeekerVersionSupportOnlySensorDownload(seekerVersionString)
	}
	return err, seekerVersionOnlySupportSensorDownload
}
...
func (h SeekerAfterCompileHook) isSeekerVersionSupportOnlySensorDownload(seekerVersionString string) bool {
	...
	lastSeekerVersionWithoutAgentDirectDownload, _ := time.Parse(dateFormatInSeekerResponse, "2018.05.01")
	...
	return !currentSeekerVersion.After(lastSeekerVersionWithoutAgentDirectDownload)
}
